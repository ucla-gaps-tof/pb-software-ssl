...
func OpenI2CFile(bus int) (*Conn, error) {
...
func OpenI2C(bus int, addr uint8) (*Conn, error) {
	f, err := os.OpenFile(fmt.Sprintf("/dev/i2c-%d", bus), os.O_RDWR, 0600)
	if err != nil {
		return nil, err
	}
...
func (c *Conn) WriteI2C(buf []byte) (int, error) {
	return c.f.Write(buf)
}

// WriteByte sends a single byte to the remote i2c device.
// The interpretation of the message is implementation dependant.
func (c *Conn) WriteByteI2C(b byte) (int, error) {
	var buf [1]byte
	buf[0] = b
	return c.f.Write(buf[:])
}
...
func (c *Conn) ReadI2C(p []byte) (int, error) {
	return c.f.Read(p)
}

// Close closes the connection to the remote i2c device.
func (c *Conn) CloseI2C() error {
	return c.f.Close()
}

// ReadReg reads a single byte from a designated register.
func (c *Conn) ReadRegI2C(addr, reg uint8) (uint8, error) {
...
func (c *Conn) WriteRegI2C(addr, reg, v uint8) error {
	if err := c.addr(addr); err != nil {
		return err
	}
...
func (c *Conn) ReadWordI2C(addr, reg uint8) (uint16, error) {
	if err := c.addr(addr); err != nil {
		return 0, err
	}
...
func (c *Conn) WriteWordI2C(addr, reg uint8, v uint16) error {
	if err := c.addr(addr); err != nil {
		return err
	}
...
func (c *Conn) ReadBlockDataI2C(addr, reg uint8, buf []byte) error {
...
func (c *Conn) WriteBlockDataI2C(addr, reg uint8, buf []byte) error {
	if len(buf) > int(i2cSMBusBlockMax) {
		return errSMBusBlockDataMax
	}
